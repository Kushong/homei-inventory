...
export default function NotesPage() {
  ...
  async function persistOrder() {
    ...
    const rest = cur.filter((n) => !n.pinned);
    const changed = [];
    rest.forEach((n, i) => {
      if (n.sort_order !== i) changed.push({ id: n.id, order: i });
    });
    if (!changed.length) return;
    setNotes((arr) => {
      const pinned = arr.filter((n) => n.pinned);
      const r = arr.filter((n) => !n.pinned).map((n, i) => ({ ...n, sort_order: i }));
      return [...pinned, ...r];
    });
    try {
      await Promise.all(
        changed.map((c) =>
          supabase.from('private_notes').update({ sort_order: c.order }).eq('id', c.id)
        )
      );
    } catch (err) {
  ...
  const status =
    saving ? '저장 중…' : dirty ? '수정됨 (저장 안 됨)' : sel && sel !== 'new' ? '저장됨' : '';
  ...
  function renderRow(n, opts) {
    ...
    const isOver = draggable && overIndex === index;
    return (
      <div
        key={n.id}
        draggable={draggable}
        ...
        style={{
          ...
          borderRadius: 10,
          border: '1px solid ' + (active ? COL.ink : isOver ? COL.accent : COL.border),
        ...
        <div style={{ width: 300, flexShrink: 0 }}>
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {pinnedNotes.map((n) => renderRow(n, null))}
              {restNotes.map((n, index) => renderRow(n, { draggable: true, index }))}
            </div>
          )}
        </div>
